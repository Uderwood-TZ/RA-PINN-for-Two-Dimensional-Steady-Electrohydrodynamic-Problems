...
CONFIG = {
    ...
    "save_root": "results_electrothermal_lstm_pinn",
...
class LSTMPINN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.seq_len = cfg.get("seq_len", 2)
        self.input_size = cfg.get("input_size", 1)
        self.hidden_size = cfg.get("lstm_hidden_size", 128)
        self.num_layers = cfg.get("lstm_num_layers", 2)
        self.decoder_hidden_dim = cfg.get("decoder_hidden_dim", 128)
        self.decoder_num_layers = cfg.get("decoder_num_layers", 3)
        self.output_dim = cfg.get("output_dim", 5)
        self.activation = cfg.get("activation", "tanh")
        ...
        for _ in range(self.decoder_num_layers):
            decoder_layers.append(nn.Linear(in_dim, self.decoder_hidden_dim))
    ...
    def build_sequence(self, x, y):

        seq = torch.stack([x, y], dim=1)  # [N,2,1]
        return seq

    def forward(self, x, y):
        seq = self.build_sequence(x, y)
...
def train(model, cfg, dirs, p_ref):
    ...
    def compute_supervised_mse(x, y, target):
        model.eval()
        with torch.no_grad():
            pred = model(x, y)
    ...
    def compute_validation_details():
        val_batch = {
            ...
            "x_train": batch["x_val"],
            "y_train": batch["y_val"],
            "target_train": batch["target_val"],
            "x_val": batch["x_val"],
            "y_val": batch["y_val"],
            "target_val": batch["target_val"],
        }

        model.eval()
        val_total, val_pde, val_bc, val_data, val_pde_detail = compute_total_loss(model, val_batch, cfg)
        model.train()
    ...
    for epoch in range(1, cfg["epochs"] + 1):
        ...
        train_mse = compute_supervised_mse(
            batch["x_train"], batch["y_train"], batch["target_train"]
        )
        val_mse = compute_supervised_mse(
            batch["x_val"], batch["y_val"], batch["target_val"]
        )

        val_total, val_pde, val_bc, val_data, val_pde_detail = compute_validation_details()
        ...
        row = {
            ...
            "val_loss": float(val_mse.item()),
            "val_total_loss": float(val_total.item()),
            "val_pde_loss": float(val_pde.item()),
            "val_bc_loss": float(val_bc.item()),
            "val_data_loss": float(val_data.item()),
            "mass_loss": float(pde_detail["loss_cont"].item()),
            "ux_loss": float(pde_detail["loss_u"].item()),
            "vy_loss": float(pde_detail["loss_v"].item()),
            "T_eq_loss": float(pde_detail["loss_T"].item()),
            "C_eq_loss": float(pde_detail["loss_c"].item()),
        ...
        if epoch == 1 or epoch % cfg["print_every"] == 0 or epoch == cfg["epochs"]:
            print(
                ...
                f"Mass={pde_detail['loss_cont'].item():.6e} | "
                f"Ux={pde_detail['loss_u'].item():.6e} | "
                f"Vy={pde_detail['loss_v'].item():.6e} | "
                f"T={pde_detail['loss_T'].item():.6e} | "
                f"C={pde_detail['loss_c'].item():.6e} | "
                f"ValTotal={val_total.item():.6e} | "
                f"ValPDE={val_pde.item():.6e} | "
                f"ValData={val_data.item():.6e} | "
                f"ValBC={val_bc.item():.6e} | "
                f"TrainMSE={train_mse.item():.6e} | "
                f"ValMSE={val_mse.item():.6e} | "
                f"LR={optimizer.param_groups[0]['lr']:.3e} | "
                f"EpochTime={epoch_time:.3f}s | "
                f"TrainTime={cumulative_time:.3f}s"
            )
    ...
    print(f"Training time: {cumulative_time:.6f} s")
