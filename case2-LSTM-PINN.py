...
class Config:
    ...
    n_boundary_each_side: int = 500  # total = 4 * this
...
class LSTMPINN(nn.Module):
    ...
    def __init__(self, cfg: Config):
        ...
        fused_input_dim = cfg.lstm_hidden_size * self.num_directions + 2
        ...
        for _ in range(cfg.fc_num_layers):
            layers.append(nn.Linear(in_dim, cfg.fc_hidden_dim))
            layers.append(act_cls())
            if cfg.dropout > 0:
                layers.append(nn.Dropout(cfg.dropout))
            in_dim = cfg.fc_hidden_dim
    ...
    def build_sequence(self, xy: torch.Tensor) -> torch.Tensor:
        ...
        x = xy[:, 0:1]
        y = xy[:, 1:2]
        return torch.stack([x, y], dim=1)  # (batch, 2, 1)
...
def compute_pde_loss_batched(model: nn.Module, dataset: Dict[str, torch.Tensor],
    ...
    """
    Each epoch, use a random subset of collocation points, then process them in small batches.
    """
...
def compute_losses(model: nn.Module, dataset: Dict[str, torch.Tensor], cfg: Config, epoch: int) -> Dict[str, torch.Tensor]:
    """
    Total loss:
        L = lambda_pde * L_pde + lambda_data * L_data + lambda_bc * L_bc
    """
...
def compute_metrics_numpy(pred: np.ndarray, true: np.ndarray) -> Dict[str, float]:
    """Compute MSE, RMSE, MAE, relative L2."""
    diff = pred - true
...
def pde_residual_fields_on_grid(model: nn.Module, dataset: Dict[str, torch.Tensor], cfg: Config) -> Dict[str, np.ndarray]:
    """
    Compute PDE residual fields on full grid in chunks for plotting.
    Source terms are recomputed on grid chunks from exact solution.
    """
...
def plot_results(model: nn.Module, dataset: Dict[str, torch.Tensor], cfg: Config, dirs: Dict[str, str]) -> None:
    """
    Save prediction/true/error field plots and residual plots.
    Every plotted field has a matching txt file.
    """
...
def train_model(model: nn.Module, dataset: Dict[str, torch.Tensor],
    ...
    def compute_supervised_loss_float(xy: torch.Tensor, uvTpc: torch.Tensor,
                                      batch_size: int, shuffle: bool = False, seed: int = 42) -> float:
        total = 0.0
        n = xy.shape[0]
        nb = 0
        model.eval()
        with torch.no_grad():
            for idx in batch_iterator(n, batch_size, shuffle=shuffle, seed=seed):
                pred = model(xy[idx])
                total += torch.mean((pred - uvTpc[idx]) ** 2).item()
                nb += 1
        return total / max(nb, 1)

    def compute_validation_details(epoch: int):
        model.eval()

        val_pde = compute_pde_loss_batched(
            model,
            dataset,
            cfg,
            epoch_seed=cfg.seed + 30000 + epoch
        )

        val_data = compute_supervised_loss_float(
            dataset["xy_val"],
            dataset["uvTpc_val"],
            batch_size=cfg.data_batch_size,
            shuffle=False,
            seed=cfg.seed + 40000 + epoch
        )

        val_bc = compute_supervised_loss_float(
            dataset["xy_boundary"],
            dataset["uvTpc_boundary"],
            batch_size=cfg.bc_batch_size,
            shuffle=False,
            seed=cfg.seed + 50000 + epoch
        )

        val_total = (
            cfg.lambda_pde * val_pde["loss_pde"].item()
            + cfg.lambda_data * val_data
            + cfg.lambda_bc * val_bc
        )

        return val_total, val_pde, val_data, val_bc
    ...
    logger.log(f"LSTM input organization: (batch, seq_len=2, input_size=1), with steps [x, y]")
    logger.log("-" * 120)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        if epoch == 1 or epoch % cfg.print_every == 0 or epoch == cfg.epochs:
            val_total, val_pde, val_data, val_bc = compute_validation_details(epoch)
            ...
            logger.log(
                ...
                f"ValTotal={val_total:.6e} | "
                f"ValPDE={val_pde['loss_pde'].item():.6e} | "
                f"ValData={val_data:.6e} | "
                f"ValBC={val_bc:.6e} | "
...
def main():
    ...
    # Pressure zero-mean correction on evaluation grid
    p_mean = compute_pressure_mean_on_grid(CFG, device)
