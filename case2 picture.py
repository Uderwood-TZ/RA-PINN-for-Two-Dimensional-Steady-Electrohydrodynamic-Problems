...
def plot_loss_panel(fig: plt.Figure, spec) -> None:
    ...
    ax.set_title("Case 2", fontsize=9, pad=3)
...
def plot_comparison_figure(
    ...
    if len(fields) == 0:
        raise RuntimeError("没有检测到可绘制字段，请确认存在类似 c_true、c_pred、c_error 这类文件。")
    ...
    fig.suptitle("Case 2: F4-style comparison", fontsize=13, y=0.985)
