...
class LSTMLayer(nn.Module):
    ...
    def forward(self, seq: torch.Tensor) -> torch.Tensor:
        # Disable CuDNN to support double backward for PINN second derivatives
...
class LSTMPINN(nn.Module):
    ...
    def forward(self, xy: torch.Tensor) -> torch.Tensor:
        ...
        lstm_feat = self.lstm_layer(seq)
        fused = torch.cat([lstm_feat, xy], dim=1)
...
def train_model(model: nn.Module, dataset: Dict[str, torch.Tensor],
    ...
    if cfg.use_lbfgs:
        ...
        history["loss_mass"].append(lbfgs_losses["loss_mass"].item())
        ...
        logger.log(
            ...
            f"Mass={lbfgs_losses['loss_mass'].item():.6e} | "
