...
class ResidualAttentionBlock(nn.Module):
    ...
    def __init__(self, hidden_dim: int, activation: str = "tanh", dropout: float = 0.0, learnable_alpha: bool = True):
        super().__init__()
        if activation.lower() == "tanh":
            act_cls = nn.Tanh
        elif activation.lower() == "relu":
        ...
        self.gate = AttentionGate(hidden_dim, activation=activation)
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        return z + self.alpha * (self.main(z) * self.gate(z))
