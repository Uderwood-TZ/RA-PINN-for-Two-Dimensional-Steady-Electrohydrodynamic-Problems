...
CONFIG = {
    ...
    "save_root": "outputs_pure_pinn_concentration_electric",
...
def compute_bc_loss(model, batch):
    pred_b = model(batch["x_b"], batch["y_b"])
    target_b = batch["target_b"]

    u_pred, v_pred, T_pred, p_pred, c_pred = split_fields(pred_b)
    u_true, v_true, T_true, p_true, c_true = split_fields(target_b)
...
def compute_pressure_anchor_loss(model, batch, cfg):

    if not cfg["use_pressure_anchor"]:
        zero = torch.tensor(0.0, device=batch["x_f"].device)
        return zero

    x_ref = batch["x_p_anchor"]
    y_ref = batch["y_p_anchor"]
    p_true = batch["p_anchor_true"]

    pred = model(x_ref, y_ref)
    p_pred = pred[:, 3:4]
    return mse_loss(p_pred, p_true)
...
def compute_total_loss(model, batch, cfg):
    ...
    loss_p_anchor = compute_pressure_anchor_loss(model, batch, cfg)

    total_loss = (
        cfg["lambda_pde"] * loss_pde
        + cfg["lambda_bc"] * loss_bc
        + cfg["lambda_data"] * loss_data
        + cfg["lambda_p_anchor"] * loss_p_anchor
    )

    return total_loss, loss_pde, loss_bc, loss_data, loss_p_anchor, pde_detail, bc_detail, data_detail
...
def plot_loss_curves(log_df, figure_dir):
    ...
    plt.plot(log_df["epoch"], log_df["p_anchor_loss"], label="p_anchor_loss")
    plt.yscale("log")
    plt.xlabel("epoch")
    plt.ylabel("loss")
...
def build_training_batch(cfg, device):
    ...
    x_p_anchor = torch.tensor([[cfg["p_anchor_x"]]], dtype=torch.float32, device=device)
    y_p_anchor = torch.tensor([[cfg["p_anchor_y"]]], dtype=torch.float32, device=device)
    p_anchor_true = p_exact(x_p_anchor, y_p_anchor).detach()
    ...
    batch = {
        ...
        "x_p_anchor": x_p_anchor,
        "y_p_anchor": y_p_anchor,
        "p_anchor_true": p_anchor_true,
    }
    return batch
...
def train(model, cfg, dirs):
    ...
    for epoch in range(1, cfg["epochs_adam"] + 1):
        ...
        total_loss, loss_pde, loss_bc, loss_data, loss_p_anchor, pde_detail, bc_detail, data_detail = compute_total_loss(model, batch, cfg)
        total_loss.backward()
        optimizer.step()
        ...
        row = {
            ...
            "p_anchor_loss": float(loss_p_anchor.item()),
    ...
    if cfg["use_lbfgs"]:
        ...
        def closure():
            ...
            total_loss_lbfgs, _, _, _, _, _, _, _ = compute_total_loss(model, batch, cfg)
    ...
    save_curve_txt(
        log_df,
        ["epoch", "pde_loss", "bc_loss", "data_loss", "p_anchor_loss", "loss_cont", "loss_u", "loss_v", "loss_T", "loss_c"],
...
def main():
    try:
        ...
        model = PurePINN(CONFIG).to(device)
